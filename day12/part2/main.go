...
func traverse(node string, visited map[string]int, graph map[string][]string, path []string) {
    ...
    for _, edge := range(graph[node]) {
        ...
        if dst != "end" && dst != "start" && !isUpper(dst) {
            twicePassed := false // determine if a single small cave has been visited twice already

            // check for all entries in the visited map to determine if we have been to one twice yet
            for key, value := range(visited) {
                if value == 2 && !isUpper(key) {
                    twicePassed = true
                    break
                }
            }

            // allowed to go into small caves once if we have already visisted a small cave twice
            if twicePassed {
                if visited[dst] < 1 {
                    traverse(dst, visited, graph, path)
                }
            } else {
                // visit any small case twice
                if visited[dst] < 2 {
                    traverse(dst, visited, graph, path)
                }
            }
