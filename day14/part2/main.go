...
func main() {
    ...
    steps := 10
    for i := 0; i < steps; i++ {
        pairs := make(map[string][]int) // hash mapping pairs to a list of their indexes (location)
        ...
        for key, indexes := range(pairs) {
            if hash[key] != "" {
                // for each instance of the pair
                for _, index := range(indexes) {
                    val := []rune(hash[key]) // split hash entry into 2 characters
                    polymer = append(polymer[:index + 1], polymer[index:]...) // add extra space for new character
                    polymer[index] = val[0] // replace new spot with entry in hash
